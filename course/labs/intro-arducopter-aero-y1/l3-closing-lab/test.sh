#!/usr/bin/env bash
# Lab L3 — headless agent-facing test harness.
#
# Starts the ArduCopter SITL binary directly (no sim_vehicle.py, no MAVProxy,
# no X11 required), then runs test.py which executes Step A (clean run) and
# Step B (EKF failsafe injection via SIM_GPS1_ENABLE=0) in one session.
#
# Usage (from any directory):
#   bash course/labs/intro-arducopter-aero-y1/l3-closing-lab/test.sh
#
# Requirements:
#   - build/sitl/bin/arducopter must exist
#   - pymavlink must be importable
#
# Exit codes mirror test.py / run_lab.py:
#   0   PASS (both steps)
#   1   Step A: critical statustext or disarm timeout
#   2   Step B: EKF variance STATUSTEXT not seen
#   3   Step B: mode did not change to LAND
#   4   Step B: disarm timeout
#   5   Step B: ERR row not found in dataflash log
#   10  connection failure, arm failure, or binary not found
set -e
set -u
...
# Run from the repo root so SITL writes dataflash logs to logs/ there.
# run_lab.py's find_latest_log() searches "." which must be the repo root.
cd "${REPO_ROOT}"
...
echo "[test.sh] SITL PID=${SITL_PID}, waiting up to 15 s for TCP 5760 to open ..."
WAIT=0
while [ "${WAIT}" -lt 15 ]; do
    if python3 -c "
import socket, sys
