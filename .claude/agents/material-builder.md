...
- **No frame overflow — `[allowframebreaks]` is mandatory on instructor-only-augmented frames.** A frame whose instructor edition includes `\instnote{}`, `\insttiming{}`, or `\instadvanced{}` content has *more* text than the student edition of the same frame. Even when the student edition fits, the instructor edition can overflow the bottom of the slide (text gets clipped at the page margin without warning — pdflatex does NOT error). Two-part rule, both required:
  1. **Source-side**: every frame that contains *any* instructor-only macro must declare `\begin{frame}[allowframebreaks]{...}`. This makes the frame split into multiple pages automatically when content exceeds one page. Apply this to every such frame, not just ones you suspect — student/instructor parity means you cannot eyeball which will overflow once `\instnote{}` is rendered.
  2. **Build-side audit**: after compiling each instructor PDF, run `pdfinfo <file>.pdf | grep "Page size"` and `pdftotext -layout <file>.pdf - | awk '/\f/{p++} END{print p}'` to confirm logical page count matches the frame count plus expected breaks. Spot-check 3 instructor frames per deck by rendering them at a known DPI and inspecting that the last line of body content sits inside the page area, not flush against the bottom margin. If any frame's last line is clipped or sits within 5 mm of the bottom, that frame must be marked `[allowframebreaks]` and recompiled. Note this audit in the final report.

  Do NOT shrink fonts (`\footnotesize`, `\small`) as a workaround — that breaks visual consistency with the student edition and degrades projector readability. `[allowframebreaks]` is the canonical Beamer mechanism; use it.
